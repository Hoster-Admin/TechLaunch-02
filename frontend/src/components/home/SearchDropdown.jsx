...
export default function SearchDropdown({ query, onClose }) {
  ...
  useEffect(() => {
    if (!query.trim()) { setProducts([]); setPeople([]); return; }

    clearTimeout(debounceRef.current);
    debounceRef.current = setTimeout(async () => {
      setLoading(true);
      try {
        const [prodRes, peopleRes] = await Promise.allSettled([
  ...
  const noResults = !loading && !products.length && !people.length;
