...
const forgotPassword = async (req, res, next) => {
  try {
    ...
    if (!rows.length) return res.json({ success:true, message:'If that email exists, a reset link has been sent' });
    ...
    const resetLink = `${process.env.APP_URL || 'https://tlmena.com'}/reset-password?token=${token}`;
    sendPasswordResetEmail({ to: email, resetLink })
...
module.exports = { register, login, refresh, logout, getMe, checkActivationToken, activateAccount, forgotPassword, resetPassword };
