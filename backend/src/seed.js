...
async function run() {
  ...
  await q(`CREATE TABLE IF NOT EXISTS community_tags (
    ...
    color VARCHAR(20) NOT NULL DEFAULT '#E8621A', created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
  )`);
  await q(`CREATE TABLE IF NOT EXISTS community_posts (
    id SERIAL PRIMARY KEY, author_id INTEGER NOT NULL,
    type VARCHAR(20) NOT NULL CHECK (type IN ('post','article')),
    status VARCHAR(20) NOT NULL DEFAULT 'draft' CHECK (status IN ('draft','published')),
    title TEXT, body TEXT NOT NULL DEFAULT '', tag_id INTEGER,
