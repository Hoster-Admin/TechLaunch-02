...
admin.get('/export/:type', async (req, res) => {
  try {
    ...
    if (type === 'products') {
      ...
      const params = [];
      const dateClause = buildDateFilter('created_at', params);
      const result = await q(`SELECT name,type,country,industry,stage,aum,verified,website,created_at FROM entities${dateClause ? ' WHERE 1=1' + dateClause : ''} ORDER BY created_at DESC`, params);
      rows = result.rows;
      headers = ['Name','Type','Country','Industry','Stage','AUM','Verified','Website','Created At'];
      filename = 'entities';
